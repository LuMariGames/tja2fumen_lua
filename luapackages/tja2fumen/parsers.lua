...
function parse_fumen(path, exclude_empty)
    local f = assert(io.open(path, "rb"))
    local size = f:seek("end")
    f:seek("set", 0)

    local header = FumenHeader.new()
    header:parse_header_values(f:read(520))

    local song = FumenCourse.new{ header = header }

    for i=1, header.b512_b515_number_of_measures do
        local ms = read_struct(f, header.order, "ffBBHiiiiiii")
        ...
        local measure = FumenMeasure.new{
            ...
            barline      = (ms[4] ~= 0),
            padding1     = ms[5],
            branch_info  = { ms[6], ms[7], ms[8], ms[9], ms[10], ms[11] },
            padding2     = ms[12],
        }

        -- Parse branches
        for _, branch_name in ipairs(BRANCH_NAMES) do
            local bs = read_struct(f, header.order, "HHf")
            local total_notes = bs[1]
            ...
            for i = 1, total_notes do
                local ns = read_struct(f, header.order, "ififHHf")
                local note_type = ns[1]
                ...
                -- Balloon / Kusudama
                if note_type == 0xA or note_type == 0xC then
                    note.hits         = ns[5]
                    note.hits_padding = ns[6]
                else
                    song.score_init   = ns[5]
                    song.score_diff   = math.floor(ns[6] / 4)
                    note.score_init   = song.score_init
                    note.score_diff   = song.score_diff
                end
                ...
                if note_type == 0x6 or note_type == 0x9 or note_type == 0x62 then
                    note.drumroll_bytes = f:read(8)
                end
    ...
    -- Remove empty measures if requested
    if exclude_empty then
        local filtered = {}
        for _, m in ipairs(song.measures) do
            local n = m.branches.normal.length
            local p = m.branches.professional.length
            local x = m.branches.master.length
            if n > 0 or p > 0 or x > 0 then
                table.insert(filtered, m)
            end
        end
        song.measures = filtered
    end
